...
def clean_upload_dirs():
    for folder in ['uploads', 'xml_outputs']:
        dir_path = Path(settings.MEDIA_ROOT) / folder
        dir_path.mkdir(parents=True, exist_ok=True)  # Ensure folder exists
...
def home(request):
    ...
    if request.method == 'POST' and request.FILES.get('excel_file'):
        ...
        if excel_file.size > 5 * 1024 * 1024:
            ...
            output_dir = Path(settings.MEDIA_ROOT) / 'xml_outputs'
            ...
            if hasattr(talend_main, 'process_excel_schema'):
                try:
                    ...
                    if xml_result:
                        # Full absolute URL ensures it works both locally and in deployment
                        xml_download_url = request.build_absolute_uri(settings.MEDIA_URL + f'xml_outputs/{xml_filename}')
